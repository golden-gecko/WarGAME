...
void Weapon::fire() {
	...
	if ((m_reloadTime >= m_weaponDesc->getMaxReloadTime()) && ((WORD)m_ammunition)) {

		m_reloadTime = 0.0f;

		m_ammunition--;

		Object* o = ObjectManager::getSingletonPtr()->createProjectile((ProjectileDesc*)ObjectDescManager::getSingletonPtr()->getByName("missile"));

		if (o) {

			o->setLinearVelocity(getDirection() * m_weaponDesc->getMaxPower() / o->getMaxHitPoints() + Ogre::Vector3(0.0f, -0.1f, 0.0f));
			o->setPosition(getWorldCenter());
